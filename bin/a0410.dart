/*
* 문제
두 개의 정수 배열 nums1과 nums2가 주어집니다. 각 배열의 고유한 원소들 중 서로의 배열에 없는 원소를 찾아 반환하세요.

nums1에 있고 nums2에는 없는 모든 고유한 요소의 리스트와,
nums2에 있고 nums1에는 없는 모든 고유한 요소의 리스트를 반환해야 합니다.

결과는 [answer1, answer2]의 형태로 반환되며,
answer1은 nums1의 고유한 원소 리스트이고, answer2는 nums2의 고유한 원소 리스트입니다.
...
class Solution {
  List<List<int>> findDifference(List<int> nums1, List<int> nums2) {
    ...
    for (int n in nums1.toSet()) {
      if (!nums2.contains(n)) {
        unique1.add(n);
      }
    }
    for (int n in nums2.toSet()) {
      if (!nums1.contains(n)) {
        unique2.add(n);
      }
    }
